...
import {
  TrendingUp, TrendingDown, Users, ChevronRight, Shield, Plus,
  Bell, Settings, Loader, AlertCircle,
  ArrowUpRight, ArrowDownLeft, CreditCard,
  BarChart2, Wheat, Activity, Minus,
} from 'lucide-react';
...
// ── ZiG sparkline — pure SVG, no library ──────────────────
// We generate a synthetic but realistic 30-day ZiG/USD rate
// trend from the live rate. This shows ZiG stability vs USD.
    ...
    >
      <defs>
        <linearGradient id="zigFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stopColor={up ? '#f59e0b' : '#ef4444'} stopOpacity="0.25" />
          <stop offset="100%" stopColor={up ? '#f59e0b' : '#ef4444'} stopOpacity="0"    />
        </linearGradient>
      </defs>
      {/* Fill area */}
  ...
  return (
    <svg width="128" height="128" viewBox="0 0 128 128">
      ...
      >
        {score >= 80 ? 'Excellent' :
         score >= 60 ? 'Good' :
         score >= 40 ? 'Building' : 'Starting'}
      </text>
    </svg>
  );
}

// ── Mini bar chart for score factors ──────────────────────
function FactorBar({
  label,
  value,
  max,
  ...
  return (
    <div>
      <div style={{
        ...
        <span style={{ fontSize: '0.72rem', color: '#57534e', fontWeight: '500' }}>
          {label}
        </span>
        <span style={{ fontSize: '0.72rem', fontWeight: '700', color: '#1c1917' }}>
          {value}/{max}
        </span>
      </div>
      <div style={{
        height: '7px', background: 'rgba(168,132,90,0.1)',
        ...
        <div style={{
          ...
          background: pct >= 80
            ...
            : pct >= 50
            ? 'linear-gradient(to right, #a8845a, #d4af37)'
            : 'linear-gradient(to right, #a8845a, #c8975a)',
          borderRadius: '4px',
...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Store attendant redirect
      if (profileData?.user?.is_store_attendant) {
  ...
  const maxLoan       = vimbisoScore?.max_loan_eligible?.replace('US$', '') ||
                        vimbisoScore?.max_loan_amount_usd || 0;
  const activeMukando = mukandoGroups.filter((g) => g.status === 'active');

  // ZiG data from trust engine
  ...
  const inflationNote = zigHealth?.zig_health_note            ?? '';
  ...
  // ZiG purchasing power comparison
  // How much ZiG the user would need vs last month (approx 3% monthly inflation)
  const zigLastMonth  = zigBalance * 0.97;
  const zigGain       = zigBalance - zigLastMonth;
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          ...
          <span style={{
            ...
            color: '#3a2a1c', letterSpacing: '0.06em',
          }}>
            BATANA
          </span>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <button style={{
            ...
            cursor: 'pointer', padding: '0.5rem', color: '#78716c',
          }}>
            <Bell size={20} />
          </button>
          <button style={{
      ...
      <div style={{ padding: '1.5rem', paddingBottom: '6rem' }}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <h1 style={{
            fontSize: '1.75rem', fontWeight: '800', color: '#1c1917',
        ...
        {!user?.is_verified && (
          <button
            ...
            style={{
              width: '100%',
              background: 'linear-gradient(135deg, #1c1917, #3a2a1c)',
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.55)', lineHeight: '1.4' }}>
                {user?.verification_submitted
                  ...
                  : 'Unlock savings, loans, mukando and insurance'}
              </div>
            </div>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{
              ...
              {goldGrams.toFixed(4)}
              <span style={{ fontSize: '1.25rem', color: 'rgba(255,255,255,0.7)', marginLeft: '0.3rem' }}>
                g gold
              </span>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                fontSize: '0.875rem', color: '#f59e0b', fontWeight: '700',
              }}>
                US${goldValueUsd.toFixed(2)}
              </div>
              <div style={{
                fontSize: '0.875rem', color: 'rgba(255,255,255,0.55)', fontWeight: '600',
              }}>
                ZiG {zigBalance.toLocaleString('en', { maximumFractionDigits: 2 })}
              </div>
              {usdBalance > 0 && (
                <div style={{
                  fontSize: '0.875rem', color: 'rgba(255,255,255,0.45)', fontWeight: '600',
                }}>
                  +US${usdBalance.toFixed(2)} cash
                </div>
              )}
            </div>

            {/* ── INLINE ZIG SPARKLINE ── */}
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.5)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  ZiG / USD Rate · 30 days
                </div>
                <div style={{
                  fontSize: '0.72rem', fontWeight: '800',
                  color: '#f59e0b',
                }}>
                  1 USD = ZiG {zigRate.toFixed(2)}
                </div>
              </div>
            ...
            {/* Action buttons */}
            <div style={{
              display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.625rem',
            }}>
              {[
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      flexDirection: 'column', alignItems: 'center', gap: '0.375rem',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: '0.75rem', color: '#a8a29e', marginTop: '0.1rem' }}>
                Gold-backed · Inflation protected
              </div>
            </div>
            <button
          ...
          <div style={{
            ...
            {/* Gold price */}
            <div style={{
              background: 'linear-gradient(135deg, #faf5e8, #f5ece0)',
              borderRadius: '1rem', padding: '0.875rem',
              border: '1px solid rgba(212,175,55,0.2)',
            }}>
              <div style={{ fontSize: '0.6rem', color: '#a8845a', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>
                Gold / gram
              </div>
              <div style={{ fontSize: '1.25rem', fontWeight: '900', color: '#3a2a1c', lineHeight: '1' }}>
                US${goldPriceUsd.toFixed(2)}
              </div>
              <div style={{ fontSize: '0.7rem', color: '#a8845a', marginTop: '0.2rem', fontWeight: '600' }}>
                ZiG {goldPriceZig.toFixed(0)}
              </div>
              <div style={{
                ...
                <span style={{ fontSize: '0.65rem', color: '#748c3d', fontWeight: '600' }}>
                  Live price
                </span>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.6rem', color: '#748c3d', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>
                ZiG / USD
              </div>
              <div style={{ fontSize: '1.25rem', fontWeight: '900', color: '#1a2e0a', lineHeight: '1' }}>
                {zigRate.toFixed(2)}
              </div>
              <div style={{ fontSize: '0.7rem', color: '#748c3d', marginTop: '0.2rem', fontWeight: '600' }}>
                Official RBZ rate
              </div>
              <div style={{
                display: 'flex', alignItems: 'center', gap: '0.25rem',
            ...
            <div style={{
              background: 'linear-gradient(135deg, #f8f5ff, #ede8f5)',
              borderRadius: '1rem', padding: '0.875rem',
              border: '1px solid rgba(124,58,237,0.12)',
            }}>
              <div style={{ fontSize: '0.6rem', color: '#7c3aed', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>
                Gold protection
              </div>
              <div style={{ fontSize: '1.25rem', fontWeight: '900', color: '#3a2a1c', lineHeight: '1' }}>
                {goldGrams > 0 ? '+' : ''}{goldGrams.toFixed(4)}g
              </div>
              <div style={{ fontSize: '0.7rem', color: '#7c3aed', marginTop: '0.2rem', fontWeight: '600' }}>
            ...
            {/* ZiG vs cash comparison */}
            <div style={{
              background: 'linear-gradient(135deg, #fef5f0, #fde8dc)',
              borderRadius: '1rem', padding: '0.875rem',
              border: '1px solid rgba(234,88,12,0.12)',
            }}>
              <div style={{ fontSize: '0.6rem', color: '#ea580c', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>
                vs Cash (est.)
              </div>
              <div style={{ fontSize: '1.25rem', fontWeight: '900', color: '#3a2a1c', lineHeight: '1' }}>
                +ZiG {zigGain.toFixed(0)}
              </div>
              <div style={{ fontSize: '0.7rem', color: '#ea580c', marginTop: '0.2rem', fontWeight: '600' }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: '0.65rem', color: '#ea580c', fontWeight: '600' }}>
                  Cash loses ~3%/mo
                </span>
              </div>
            </div>
          ...
          {/* ── ZiG vs USD purchasing power bar ── */}
          <div style={{
            ...
            <div style={{ marginBottom: '0.625rem' }}>
              <div style={{
                ...
                <span style={{ color: '#748c3d', fontWeight: '700' }}>
                  Your ZiG (gold-backed)
                </span>
                <span style={{ color: '#748c3d', fontWeight: '800' }}>100%</span>
              </div>
              <div style={{
                height: '10px', background: 'rgba(116,140,61,0.12)',
                borderRadius: '5px', overflow: 'hidden',
              }}>
                <div style={{
                  height: '100%', width: '100%',
                  background: 'linear-gradient(to right, #748c3d, #9ab55a)',
                  borderRadius: '5px',
                }} />
              </div>
            </div>
            ...
            <div>
              <div style={{
                ...
                <span style={{ color: '#ea580c', fontWeight: '700' }}>
                  Unprotected cash (est.)
                </span>
                <span style={{ color: '#ea580c', fontWeight: '800' }}>~97%</span>
              </div>
              <div style={{
                height: '10px', background: 'rgba(234,88,12,0.1)',
                borderRadius: '5px', overflow: 'hidden',
              }}>
                <div style={{
                  height: '100%', width: '97%',
            ...
            <div style={{
              fontSize: '0.7rem', color: '#78716c', marginTop: '0.75rem',
              lineHeight: '1.5',
            }}>
              Gold-backed ZiG preserves your purchasing power. Unprotected cash
              loses value each month to inflation.
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: '0.7rem', fontWeight: '700', color: '#57534e', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                ZiG Rate Trend
              </span>
              <span style={{ fontSize: '0.7rem', color: '#a8845a', fontWeight: '700' }}>
                ZiG {zigRate.toFixed(2)} / USD
              </span>
            </div>
            ...
            <div style={{
              display: 'flex', justifyContent: 'space-between',
              fontSize: '0.62rem', color: '#a8a29e', marginTop: '0.4rem',
            }}>
        ...
        <div style={{
          ...
          {quickLinks.map((item) => (
            ...
            >
              {item.icon}
              <span style={{ fontSize: '0.68rem', fontWeight: '600', color: '#57534e' }}>
                {item.label}
              </span>
            </button>
          ))}
        ...
        <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              {maxLoan > 0 && (
                <div style={{
                  ...
                  <div style={{ fontSize: '0.62rem', color: '#748c3d', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '0.2rem' }}>
                    Loan eligible
                  </div>
                  <div style={{ fontSize: '1.35rem', fontWeight: '900', color: '#1a2e0a' }}>
                    US${maxLoan}
                  </div>
                </div>
              )}
              ...
              <div style={{
                fontSize: '0.75rem', color: '#57534e', lineHeight: '1.55',
              }}>
                {score >= 80
                  ? 'Excellent standing. You qualify for the highest loan tier with the lowest interest rate.'
                  : score >= 60
                  ? 'Good standing. Keep contributing to your mukando to unlock more.'
                  : score >= 40
                  ? 'Building your history. Every contribution improves your score.'
          ...
          <div style={{
            ...
            {Object.entries(factors).slice(0, 5).map(([key, value]) => (
              <FactorBar
                key={key}
                label={scoreFactorLabels[key] || key}
                value={value as number}
                max={scoreFactorMax[key] || 10}
              />
            ))}
          </div>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '1rem', color: '#a8845a',
        ...
        {activeMukando.length > 0 ? (
          <div style={{
              ...
              return (
                <div key={group.group_id}>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <div style={{
                      display: 'flex', justifyContent: 'space-between',
                      fontSize: '0.65rem', color: '#a8a29e', marginBottom: '0.3rem',
                    }}>
                      <span>Cycle progress</span>
                      <span style={{ fontWeight: '700', color: '#a8845a' }}>
                        {Math.round(progress)}%
                      </span>
                    </div>
                    <div style={{
                      height: '8px', background: 'rgba(168,132,90,0.1)',
                      borderRadius: '4px', overflow: 'hidden',
                    }}>
                      <div style={{
                        height: '100%', width: `${progress}%`,
                        background: 'linear-gradient(to right, #a8845a, #d4af37)',
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ fontSize: '0.6rem', color: '#a8845a', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '0.25rem' }}>
                        Contributed
                      </div>
                      <div style={{ fontSize: '1.1rem', fontWeight: '900', color: '#3a2a1c' }}>
                        {parseFloat(group.total_contributed?.gold_grams || 0).toFixed(4)}g
                      </div>
                      <div style={{ fontSize: '0.68rem', color: '#a8845a', fontWeight: '600', marginTop: '0.1rem' }}>
                        ZiG {parseFloat(group.total_contributed?.zig || 0).toLocaleString('en', { maximumFractionDigits: 0 })}
                      </div>
                    </div>
                    <div style={{
                      ...
                      <div style={{ fontSize: '0.6rem', color: '#748c3d', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '0.25rem' }}>
                        Score impact
                      </div>
                      <div style={{ fontSize: '1.1rem', fontWeight: '900', color: '#1a2e0a' }}>
                        +{Math.min(currentMonth * 3, 30)}pts
                      </div>
                      <div style={{ fontSize: '0.68rem', color: '#748c3d', fontWeight: '600', marginTop: '0.1rem' }}>
                        From mukando history
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <button
              onClick={() => { window.location.href = '/mukando'; }}
              style={{
                width: '100%', padding: '0.75rem',
                background: 'transparent',
                border: '1.5px solid rgba(168,132,90,0.3)',
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.875rem', fontWeight: '600', color: '#57534e', marginBottom: '0.5rem' }}>
              No mukando groups yet
            </div>
            <div style={{ fontSize: '0.8rem', color: '#78716c', marginBottom: '1.25rem' }}>
        ...
        {/* ── GROW TEASER ── */}
        <button
          onClick={() => { window.location.href = '/grow'; }}
          style={{
            width: '100%',
        ...
        >
          ...
          <div style={{ flex: 1, textAlign: 'left' }}>
            <div style={{ fontSize: '0.9rem', fontWeight: '700', color: 'white', marginBottom: '0.15rem' }}>
              Invest in GROW pools
            </div>
            <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.55)' }}>
              Fund verified farmers · Earn 15–25% returns
            </div>
          </div>
